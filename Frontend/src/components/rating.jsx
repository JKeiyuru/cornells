...
/**
 * Show average rating with stars and optional count
 */
export const showAverage = (product, showCount = true) => {
  ...
  const average =
    product.ratings.reduce((sum, rating) => sum + rating.star, 0) /
...
/**
 * Interactive star rating for leaving reviews
 */
      ...
      {[...Array(5)].map((_, i) => {
        ...
        return (
          <div key={i} className="relative">
            ...
            {filled && (
              <FaStar className="w-3 h-3 text-yellow-400 absolute inset-0" />
            )}
            {halfFilled && (
              <div className="absolute inset-0 overflow-hidden w-1/2">
...
/**
 * Rating distribution bars (for product detail pages)
 */
export const RatingDistribution = ({ ratings }) => {
  ...
  const distribution = [5, 4, 3, 2, 1].map((star) => ({
    ...
    percentage:
      (ratings.filter((rating) => rating.star === star).length /
        ratings.length) *
      100,
  }));
